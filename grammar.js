module.exports = grammar({
    ...
    rules: {
        ...
        expression: $ => choice(
            ...
            prec.right(2, seq(field('operator', '!'), $.expression)),
            prec.right(2, seq(field('operator', '~'), $.expression)),
            prec.right(2, seq(field('operator', '-'), $.expression)),
            prec.right(2, seq(field('operator', '+'), $.expression)),
            prec.left(1, seq($.type_name, field('operator', '.'), $.member)),
            prec.left(1, seq('error', '.', $.member)),
            prec.left(1, seq($.expression, field('operator', '.'), $.member)),
            ...
            prec.left(5, seq($.expression, field('operator', '>>'), $.expression)),
            prec.left(11, seq($.expression, field('operator', '<='), $.expression)), //TODO: Double check precedence of <=,<... and == 
            prec.left(11, seq($.expression, field('operator', '>='), $.expression)),
            prec.left(11, seq($.expression, field('operator', '<'), $.expression)),
            prec.left(11, seq($.expression, field('operator', '>'), $.expression)),
            prec.left(7, seq($.expression, field('operator', '!='), $.expression)),
            prec.left(7, seq($.expression, field('operator', '=='), $.expression)),
            prec.left(8, seq($.expression, field('operator', '&'), $.expression)),
            prec.left(9, seq($.expression, field('operator', '^'), $.expression)),
            prec.left(10, seq($.expression, field('operator', '|'), $.expression)),
            prec.left(1, seq($.expression, field('operator', '++'), $.expression)),
            prec.left(12, seq($.expression, field('operator', '&&'), $.expression)),
            prec.left(12, seq($.expression, field('operator', '||'), $.expression)),
            prec.right(13, seq($.expression, field('operator', '?'), $.expression, field('operator', ':'), $.expression)),
        ...
        non_brace_expression: $ => choice(
            ...
            seq('(', $.expression, ')'),
            seq(field('operator', '!'), $.expression),
            seq(field('operator', '~'), $.expression),
            seq(field('operator', '-'), $.expression),
            seq(field('operator', '+'), $.expression),
            seq($.type_name, field('operator', '.'), $.member),
            seq('error', '.', $.member),
            seq($.non_brace_expression, field('operator', '.'), $.member),
            seq($.non_brace_expression, field('operator', '*'), $.expression),
            seq($.non_brace_expression, field('operator', '/'), $.expression),
            seq($.non_brace_expression, field('operator', '%'), $.expression),
            seq($.non_brace_expression, field('operator', '+'), $.expression),
            seq($.non_brace_expression, field('operator', '-'), $.expression),
            ...
            seq($.non_brace_expression, field('operator', '&&'), $.expression),
            seq($.non_brace_expression, field('operator', '||'), $.expression),
            seq($.non_brace_expression, field('operator', '?'), $.expression, field('operator', ':'), $.expression),
